...
chg1920_d = paste("$", prettyNum(round(-salavg20_n+salavg19_n, digits=0), big.mark = ","), sep = "")

# Average salary accross years
salavg =  paste("$", round(mean(master$salary)/1000, digits = 0),"k", sep = "")


longsum_chg = longsum %>% 
  mutate(
    chg = (sec_frq - lag(sec_frq))/ lag(sec_frq) *100
    )
...
-   Led by nurses and teachers, the higher-than-expected surge can be explained by a major compensation agreement in early 2020 and the government's efforts to combat the pandemic.
...
The size of sectors vary, but Municipalities, School Boards, Hospitals, and Universities, have captured the lion share of high earners for most of the list's existence.

![Long Summary by Sector](plots/p_longsecsum.png)

## 2020 in Review

The last release of the OSL reported an increase in members by a total of `r chg1920` employees and a reduction in the average salary by `r chg1920_d`.
...
-   Teachers (\$103k) dominated the School Board sector in 2020, making up 97% of jobs in the sector.
...
This model concludes that the increase in the number of people in the OSL in 2020 was much higher than expected. While it predicted that only `r pred_20` public sector workers would earn more than \$100k, or a growth of `r predchg`%. Instead, the list featured `r count20` members (`r round((count20_n-count19_n)/count19_n*100, digits = 0)`%), surpassing expectations by `r pred_diff` employees.
...
diff20_p = round(adj20_n/count20_n*100, digits = 1)
...
The 2020 list shrinks by `r 100 - diff20_p`% when adjusting for inflation, totaling only `r adj20` members.
